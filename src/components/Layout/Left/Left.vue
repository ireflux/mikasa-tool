...
const isCollapsed = computed(() => !props.inDrawer && componentStore.leftCom)

//获取分类 - called in setup for SSR compatibility
  ...
  <div class="sidebar-inner">
    ...
    <el-scrollbar class="menu-scrollbar">
      <el-menu
        ...
        :default-openeds="isCollapsed ? [] : defaultOpeneds"
        :collapse="isCollapsed"
        :collapse-transition="true"
        background-color="transparent"
        :style="{ width: isCollapsed ? '72px' : '200px', margin: '0.5rem auto 0' }"
      >
        <el-sub-menu index="cate">
          <template #title>
            ...
            <span class="ml-2">{{ t('ui.categories') }}</span>
          </template>
          <el-menu-item-group>
              <el-menu-item
                v-for="(item,index) in toolsStore.localizedCates"
                :key="index"
        ...
        <el-menu-item index="about" @click="gotoAbout">
          ...
          <template #title>
            <span class="ml-2">{{ t('ui.about') }}</span>
          </template>
        </el-menu-item>
      </el-menu>
    </el-scrollbar>
    ...
    <!-- 广告位 -->
    <div v-if="sanitizedAdv != '' && !isCollapsed" class="ad-container" v-html="sanitizedAdv"></div>
  </div>
</template>
